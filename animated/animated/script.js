...
moodYes.addEventListener("click", () => {
  ...
  revealVisibleNow(escapeOptions);
...
function updateBirthdayCountdown() {
  ...
  setCountdownField(countdownDays, String(daysLeft).padStart(2, "0"));
  setCountdownField(countdownHours, String(hoursLeft).padStart(2, "0"));
  setCountdownField(countdownMinutes, String(minutesLeft).padStart(2, "0"));
  setCountdownField(countdownSeconds, String(secondsLeft).padStart(2, "0"));
}

function setCountdownField(el, value) {
...
function launchFireworks() {
  ...
  for (let burst = 0; burst < bursts; burst += 1) {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      spark.style.setProperty("--angle", `${(360 / count) * index}deg`);
      spark.style.setProperty("--distance", `${85 + Math.random() * 145}px`);
      spark.style.setProperty("--delay", `${burst * .28 + Math.random() * .32}s`);
...
function launchFallingHearts() {
  ...
  for (let index = 0; index < 22; index += 1) {
    ...
    heart.style.setProperty("--hsize", `${.8 + Math.random() * .9}rem`);
    heart.style.setProperty("--hdur", `${2.6 + Math.random() * 2.2}s`);
    heart.style.setProperty("--hdelay", `${Math.random() * .8}s`);
    heart.style.setProperty("--hdrift", `${(Math.random() - .5) * 120}px`);
    heart.style.setProperty("--hrot", `${(Math.random() - .5) * 140}deg`);
...
function initAmbientParticles() {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    particle.style.setProperty("--dur", `${9 + Math.random() * 10}s`);
    particle.style.setProperty("--delay", `${Math.random() * 12}s`);
    particle.style.setProperty("--drift", `${(Math.random() - .5) * 80}px`);
    particle.style.setProperty("--peak", `${.3 + Math.random() * .4}`);
...
function initReveal() {
  ...
  targets.forEach(el => {
    const parent = el.closest("[id]") || el.parentElement;
    if (!groups.has(parent)) groups.set(parent, []);
    groups.get(parent).push(el);
  });
  groups.forEach(list => list.forEach((el, i) => el.style.setProperty("--stagger-delay", `${i * 0.07}s`)));
  ...
  targets.forEach(el => {
    ...
    // If already hidden right now, reveal will be triggered by revealNow() when unhidden.
    if (!section || !section.hidden) observer.observe(el);
    else el.dataset.pendingReveal = "true";
  });
...
function initCardTilt() {
  ...
  stayCards.forEach(card => {
    card.addEventListener("mousemove", event => {
      ...
      card.style.setProperty("--tiltY", `${px * 7}deg`);
...
document.querySelectorAll(".stay-thumb").forEach(thumb => {
  thumb.addEventListener("click", () => {
    ...
    const mainImage = card.querySelector(".stay-card__main");
    if (mainImage.src.endsWith(thumb.dataset.src)) return;
    mainImage.classList.add("is-crossfading");
    setTimeout(() => {
      mainImage.src = thumb.dataset.src;
      mainImage.classList.remove("is-crossfading");
    }, 160);
  });
});
